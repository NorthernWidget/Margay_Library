...
```c++
if (!rangefinder.begin()) Logger.note("NotAnswering");
```

The word is written in the row, printed to the serial monitor as `Note: NotAnswering`, and shown as an orange pulse on the LED. Several notes in one interval are joined with `;`. Measurement columns keep `-9999` for a failed reading.
...
A new pair of files starts at each logging start, numbered from the first unused number. The data file begins with its header row. The status file (`sta`, same number) is a table of reports: `Time,Trigger,Device,Serial,HW,FW,FWCommit,Lib,LibCommit,Code,Note,Page0,Page1,Page2`. FW and FWCommit are a device's firmware patch and build commit (for the logger, its library version and commit); Lib and LibCommit are the version and commit of the library that read the device (for the logger, the sketch's commit). The commits come from the build wrapper and are blank in an IDE build. Its first row is the logger's own boot row, which records the library version, the serial number and the hardware version that the data file's first line used to carry. Rows for the sensors follow. Call `Logger.watch(sensor)` once per sensor in `setup()` (any library on NW_Core: Apis, Walrus, Haar, Libelle), and after every reading the logger writes a row for each watched sensor whose report says something happened: a report captured with the reading (trigger `report`), or a report the sensor captured at its own boot other than the reset this logger causes by powering the rail (trigger `boot`). The first reading writes a boot row for every watched sensor, so the file records each device's identity and versions. The Time column of a status row equals the Time of the data row the report belongs to.

### Margay as a device:

Margay is itself a Schema 1 device ([NW-Device-Specification](https://github.com/NorthernWidget/NW-Device-Specification), Margay appendix). It reads its identity from Page 0 and its calibration from Page 1, the top 64 bytes of its EEPROM, written by [NW-Provision](https://github.com/NorthernWidget/NW-Provision); when Page 1 is blank the library uses its built-in constants. At every reading it fills Page 2 with a reading of itself (battery, onboard environment, clock) and Page 3 with its logger state, and it keeps a Report register of its own: the first fault its self-tests found or `LoggingStarted` at boot, then `NewLogFile`, `RowNotWritten`, `ClockSet` and `BatteryWarning` as they happen, with faults on its chips (`SDCardNotAnswering`, `ClockNotAnswering`, `BME280NotAnswering`, `BatteryOutOfRange`) taking precedence. It watches itself: its rows in the status file come through the same path as a sensor's, first in each round, with its pages in the hex columns. Nothing serves these pages over a bus yet; the UART transport is Okapi's to build first.
